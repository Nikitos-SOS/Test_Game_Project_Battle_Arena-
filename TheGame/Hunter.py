...
class Hunter(Character):
    ...
    def hard__attack(self):
        ...
        typing(
            "Охотник заряжает свой лук несколько секунд и делает одиночный выстрел. Наносит: " + str(damage) + " урона")
        return damage

    def alt__attack(self):
        damage = 30 + 30 * (self.dexterity / 100)
        self.mana = self.mana - 20
        typing("Охотник направляет ветер в сторону врага, чтобы увеличить скорость атаки. Наносит: " + str(
            damage) + " урона")
        return damage
